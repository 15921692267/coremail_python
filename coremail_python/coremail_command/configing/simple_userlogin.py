...
def simple_common_user_login(user_information):
    ...
    print "http://{mail_host}/coremail/main.jsp?sid={sid}".format(mail_host=user_information["server_host"], sid=sid[0])

    print "----------------"
    print "Info: webadmin {user_name} simple point login".format(user_name=user_information["user_name"])
    print "http://{mail_host}/webadmin/~{sid}/~/usr/index_usr.jsp".format(mail_host=user_information["server_host"], sid=sid[0])
